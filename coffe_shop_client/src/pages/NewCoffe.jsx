...
export default function NewCoffe() {
  ...
  const handleNewCoffe = e => {
    ...
    // send data to server
    fetch(`http://localhost:3000/newCoffe`, {
      method: "POST",
      headers: {
      ...
      .then(res => res.json())
      .then(data => console.log("after add coffe: ", data))
  }
      ...
      <div className='bg-base-300 p-[5vw]'>
        ...
        <div className='mx-[1vw]'>
          ...
          <div className='grid md:grid-cols-2 grid-cols-1 place-content-center'>
            ...
            <fieldset className="fieldset rounded-box p-4">
              <label className="label text-base font-bold text-[#1B1A1A]">Category</label>
              <input type="text" className="input border-none bg-white w-full" placeholder="Enter coffee category" name='Category' />
            </fieldset>
            <fieldset className="fieldset rounded-box p-4">
              <label className="label text-base font-bold text-[#1B1A1A]">Details</label>
              <input type="text" className="input border-none bg-white w-full" placeholder="Enter coffee details" name='Details' />
            </fieldset>
          </div>
          <fieldset className="fieldset rounded-box p-4">
